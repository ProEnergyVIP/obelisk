...
in rec {
  ...
  compressedJs = frontend: optimizationLevel: pkgs.runCommand "compressedJs" {} ''
    ...
    for f in **/all.js; do
      ...
      '' else ''
        '${pkgs.closurecompiler}/bin/closure-compiler' --externs '${reflex-platform.ghcjsExternsJs}' -O '${optimizationLevel}' --jscomp_warning=checkVars --warning_level=QUIET --js_output_file="$dir/all.js" "$dir/all.unminified.js"
      ''}
    done
  '';
